...
export default function ContactPage() {
  ...
  const validateForm = () => {
    if (!formData.fullName.trim()) {
      setSubmitMessage('يرجى إدخال الاسم الكامل')
      setMessageType('error')
      return false
    }
    if (!formData.phoneNumber.trim()) {
      setSubmitMessage('يرجى إدخال رقم الهاتف')
      setMessageType('error')
      return false
    }
    ...
    if (!formData.treatmentType) {
      setSubmitMessage('يرجى اختيار نوع العلاج')
      setMessageType('error')
      return false
    }
    if (!formData.preferredDate) {
      setSubmitMessage('يرجى اختيار التاريخ المفضل')
      setMessageType('error')
      return false
    }
    ...
    if (!phoneRegex.test(formData.phoneNumber)) {
      setSubmitMessage('يرجى إدخال رقم هاتف صحيح')
      setMessageType('error')
      return false
    }
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.message) {
        setSubmitMessage('تم إرسال طلب الموعد بنجاح! سنتواصل معك خلال 24 ساعة.')
      ...
      if (error.code === 'ECONNABORTED') {
        setSubmitMessage('انتهت مهلة الاتصال. يرجى المحاولة مرة أخرى أو الاتصال بنا مباشرة.')
      } else if (error.response) {
        // Server responded with error
        setSubmitMessage(`خطأ في الخادم: ${error.response.data?.message || 'حدث خطأ غير متوقع'}`)
      } else if (error.request) {
        // Network error
        setSubmitMessage('خطأ في الاتصال. يرجى التحقق من اتصال الإنترنت والمحاولة مرة أخرى.')
      } else {
        setSubmitMessage('حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى أو الاتصال بنا مباشرة.')
      }
      setMessageType('error')
    } finally {
                      ...
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 bg-red-100 rounded-lg flex items-center justify-center flex-shrink-0">
                          <svg className="w-6 h-6 text-red-600" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M23.498 6.186a2.994 2.994 0 0 0-2.11-2.115C19.225 3.5 12 3.5 12 3.5s-7.225 0-9.388.571a2.994 2.994 0 0 0-2.11 2.115A29.94 29.94 0 0 0 0 12a29.94 29.94 0 0 0 .502 5.814 2.994 2.994 0 0 0 2.11 2.115C4.775 20.5 12 20.5 12 20.5s7.225 0 9.388-.571a2.994 2.994 0 0 0 2.11-2.115A29.94 29.94 0 0 0 24 12a29.94 29.94 0 0 0-.502-5.814zM9.545 15.568V8.432l6.545 3.568-6.545 3.568z"/>
                          </svg>
                        </div>
                        <div>
              ...
              <Card>
                <CardContent className="p-8">
                  ...
                  <form onSubmit={submitForm} className="space-y-6">
                    ...
                    <div>
                      <label htmlFor="treatmentType" className="block text-sm font-medium text-gray-700 mb-2">نوع العلاج المطلوب *</label>
                      <select 
                        id="treatmentType" 
                        name="treatmentType" 
                      ...
                      >
                        ...
                        <option value="علاج الشخير للبالغين">علاج الشخير للبالغين</option>
                        <option value="علاج الشخير للأطفال">علاج الشخير للأطفال</option>
                    ...
                    <div>
                      <label htmlFor="preferredDate" className="block text-sm font-medium text-gray-700 mb-2">التاريخ المفضل *</label>
                      <input
                        type="date"
                        id="preferredDate"
                        name="preferredDate"
                    ...
                    <div>
                      <label htmlFor="additionalNotes" className="block text-sm font-medium text-gray-700 mb-2">ملاحظات إضافية</label>
                      <textarea
                        rows={4}
                        id="additionalNotes"
                    ...
                    <Button 
                      type="submit"
                      size="lg" 
                      className="w-full bg-blue-600 hover:bg-primary text-white"
